...
import type { CreativeMode, CreativeRequest, LinkedInInput, PoemInput, StoryInput } from "./types";
...
const styleSafetyRule =
  "Never imitate a living author or reproduce copyrighted passages. Instead, follow high-level traits like pace, texture, emotional temperature, structure, and clarity.";

const qualityRule =
  "Favor specificity over cliche, vary sentence rhythm, and keep the writing original, coherent, and mode-appropriate.";

function optionalLine(label: string, value?: string) {
  return value?.trim() ? `${label}: ${value.trim()}` : undefined;
}

export function buildPoemPrompt(input: PoemInput): PromptParts {
  return {
    system: [
...
export function buildStoryPrompt(input: StoryInput): PromptParts {
  return {
    system: [
...
function buildArticleSummaryBlock(input: LinkedInInput) {
  return input.articles
    .map((article, index) => {
      const preview = article.text.slice(0, 4_500);
      return `Article ${index + 1}: ${article.name}\n${preview}`;
    })
    .join("\n\n---\n\n");
}

export function buildLinkedInPrompt(input: LinkedInInput): PromptParts {
  return {
    system: [
...
export function buildRewritePrompt(payload: CreativeRequest): PromptParts {
...
export function buildPrompt(payload: CreativeRequest): PromptParts {
  if (payload.action === "rewrite") {
    return buildRewritePrompt(payload);
  }

  switch (payload.mode as CreativeMode) {
    case "poem":
      return buildPoemPrompt(payload.input as PoemInput);
    case "story":
      return buildStoryPrompt(payload.input as StoryInput);
    case "linkedin":
      return buildLinkedInPrompt(payload.input as LinkedInInput);
    default:
