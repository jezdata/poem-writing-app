...
function validateCommonLength(value: string, field: string, fieldErrors: Record<string, string>) {
  if (value.length >= MAX_TEXT_INPUT) {
    fieldErrors[field] = "This field is too long. Please shorten it a little.";
  }
}

export function validatePoemInput(input: PoemInput): ValidationResult<PoemInput> {
...
export function validateStoryInput(input: StoryInput): ValidationResult<StoryInput> {
...
export function validateLinkedInInput(input: LinkedInInput): ValidationResult<LinkedInInput> {
...
export function validateModeInput<TMode extends CreativeMode>(
  mode: TMode,
  input: ModeInputMap[TMode]
): ValidationResult<ModeInputMap[TMode]> {
  if (mode === "poem") {
    return validatePoemInput(input as PoemInput) as ValidationResult<ModeInputMap[TMode]>;
  }

  if (mode === "story") {
    return validateStoryInput(input as StoryInput) as ValidationResult<ModeInputMap[TMode]>;
  }

  if (mode === "linkedin") {
    return validateLinkedInInput(input as LinkedInInput) as ValidationResult<ModeInputMap[TMode]>;
  }
...
export function validateCreativeRequest(
  payload: CreativeRequest
): ValidationResult<CreativeRequest> {
  ...
  const validatedInput = validateModeInput(
    payload.mode,
    payload.input as ModeInputMap[typeof payload.mode]
  );
  ...
  const previousOutput = sanitizeText(payload.previousOutput ?? "", 12_000);
