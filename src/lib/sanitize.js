...
export function sanitizeText(value, maxLength = MAX_TEXT_INPUT) {
...
export function sanitizeOptional(value, maxLength = MAX_TEXT_INPUT) {
...
export function sanitizePoemInput(input) {
    return {
...
export function sanitizeStoryInput(input) {
    return {
...
export function sanitizeLinkedInInput(input) {
    return {
        ownAngle: sanitizeText(input.ownAngle, 1500),
...
export function sanitizeModeInput(mode, input) {
    if (mode === "poem") {
        return sanitizePoemInput(input);
    }
    if (mode === "story") {
        return sanitizeStoryInput(input);
    }
    return sanitizeLinkedInInput(input);
}
