...
export async function generateWithOpenAI(payload) {
    ...
    try {
        ...
        if (!response.ok) {
            const apiMessage = responseBody?.error?.message || "The writing service could not complete this request.";
            if (response.status === 429) {
                throw new ApiError("The writing service is receiving too many requests right now. Please try again in a moment.", {
                    code: "rate_limited",
                    status: 429,
                    retryable: true
                });
            ...
            if (response.status >= 500) {
                throw new ApiError("The writing service had a temporary issue. Please retry in a moment.", {
