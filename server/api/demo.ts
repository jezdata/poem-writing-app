...
function createPoem(theme: string, tone: string) {
  return `Quiet Flame
...
function createStory(premise: string, tone: string) {
...
function createLinkedIn(angle: string, audience: string) {
...
export function generateDemoResponse(
  ...
  if (payload.mode === "poem") {
    const poemInput = payload.input as PoemInput;
    return {
      mode: payload.mode,
      action: payload.action,
      title: "Poem draft",
  ...
  if (payload.mode === "story") {
    const storyInput = payload.input as StoryInput;
    return {
      mode: payload.mode,
      action: payload.action,
      title: "Story draft",
  ...
  const linkedinInput = payload.input as LinkedInInput;
  return {
    mode: payload.mode,
    action: payload.action,
