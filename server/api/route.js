...
export async function handleCreativeRoute(req, res) {
    if (req.method !== "POST") {
        sendJson(res, 405, {
            ok: false,
            error: {
    ...
    try {
        ...
        if (!validation.ok || !validation.value) {
            throw new ApiError(validation.formError ?? "Please check your inputs and try again.", {
        ...
        sendJson(res, apiError.status, {
            ...
            error: {
                code: apiError.code,
                message: apiError.code === "missing_environment"
